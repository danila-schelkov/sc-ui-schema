...
Вы можете использовать `https://ext.nulls.gg/mods/schema/ui.schema.json` в качестве `$schema` в любом JSON и TOML файле.
...
3. Добавление `$schema` — в конвертированный JSON автоматически добавляется поле `$schema`, указывающее на локальную схему (`src/ui.schema.json`).
